...
export default function LeadsMap({ 
    ...
    return (
      <div className="flex h-full items-center justify-center bg-gray-50">
        <div className="text-center">
          ...
          <div className="text-sm text-gray-500">Initializing Leaflet</div>
        </div>
      </div>
    );
  }
  ...
  if (properties.length === 0) {
    return (
      <div className="flex h-full items-center justify-center bg-gray-50">
        <div className="text-center">
          <div className="mb-2 text-lg font-semibold text-gray-700">No Properties to Display</div>
  ...
  return (
    ...
    >
          ...
          return (
            <Marker
              ...
              eventHandlers={{
                click: () => {
                  if (onPropertyClick) {
                    onPropertyClick(property.id, property.lead?.id);
                  }
                },
              }}
            >
              <Popup>
                <div className="min-w-[200px] p-2">
                  ...
                  {property.lead ? (
                    <div className="space-y-1 text-sm">
                      ...
                      <div className="mt-2 flex items-center gap-2">
                        ...
                        {property.lead.priority_score !== null && (
                          <span className="text-xs text-gray-600">
                            Priority: {property.lead.priority_score}
                          </span>
                        )}
                      </div>
                    </div>
                  ...
                  {onPropertyClick && (
                    <button
                      onClick={() => onPropertyClick(property.id, property.lead?.id)}
                      className="mt-3 w-full rounded bg-[var(--color-primary-gold)] px-3 py-1.5 text-sm font-bold text-[var(--color-navy)] transition hover:brightness-95"
                    >
                      View Details
                    </button>
                  )}
      ...
      <div className="leaflet-bottom leaflet-right">
        <div className="leaflet-control bg-white p-3 shadow-md">
