...
<Response>
  ...
  // timeout="10" means ring for ~10 seconds before going to action URL (voicemail)
  const whisperMessage = `Incoming call from ${callerName}, a lead from Rush N Dush. Press 1 to accept.`;
  
  return `<?xml version="1.0" encoding="UTF-8"?>
<Response>
  <Dial answerOnBridge="true" timeout="10" action="${process.env.NEXT_PUBLIC_SITE_URL || ''}/api/twilio/voice/no-answer">
